...
def test_bib_error(mocker, capsys):
    ...
    mock_convert = mocker.patch(
        "domdb.cli.bib.convert_json_to_bib", side_effect=ConversionError("error")
    )
...
def test_j2e_error(mocker, capsys):
    ...
    mock_convert = mocker.patch(
        "domdb.cli.j2e.convert_json_to_evid", side_effect=EvidConversionError("error")
    )
    with pytest.raises(SystemExit) as excinfo:
        j2e(5, "dir", "out")
    assert excinfo.value.code == 1
...
def test_download_error(mocker, capsys):
    ...
    mock_load = mocker.patch(
        "domdb.cli.download.load_next_batch", side_effect=DownloadError("error")
    )
    with pytest.raises(SystemExit) as excinfo:
        download("dir")
    assert excinfo.value.code == 1
...
def test_download_unexpected_error(mocker, capsys):
    ...
    mock_load = mocker.patch(
        "domdb.cli.download.load_next_batch", side_effect=Exception("unexpected")
    )
    with pytest.raises(SystemExit) as excinfo:
        download("dir")
    assert excinfo.value.code == 1
